...
> **状态**：U.1–U.9 ✅ 完成（2026-06-01 U-findings.md 验证）；U.10 ⏸ 推迟（需 dev stack）
> **实现 commits**：`7c5d7ea` (models+services+tools+skills) + `e9b9e81` (web pages) + `1fcf4eb` (AuditPanel+ApprovalCountdown) + `17b560d` (E2E coverage specs)
> **后续补完**：`2c564c7` 修复 11 个 pre-existing test failures（TestAgentChat 401 + TestOperationsAPIRoutes 404）

| # | 任务 | 文件 | 估时 | 状态 |
|---|---|---|---:|---|
| U.1 | OperationLog 加 `error_category` / `immutable` / `superseded_by` | `app/models/operation_log.py` | 1h | ✅ |
| U.2 | ApprovalService 接管 HumanLoop 持久化 | `app/services/approval_service.py` | 2h | ✅ |
| U.3 | 重构 `app/agents/human_loop.py` 用 ApprovalService | `app/agents/human_loop.py` | 1h | ✅ |
| U.4 | `operation_stats_hourly` 物化表 + 5min UPSERT 任务 | `app/services/aggregation_service.py` | 2h | ✅ |
| U.5 | `GET /api/v1/audit/logs` 端点 + 过滤 | `app/api/audit.py` | 1h | ✅ |
...
| U.10 | E2E 回归 + 覆盖率守门 ≥ 90% | Playwright suite + CI | 1h | ⏸ |

**U 退出标准**：
- [x] 进程重启后 pending 审批不丢失 (DB-backed Approval)
- [x] Dashboard `/operations/summary` 响应 < 200ms (操作数表 `operation_stats_hourly`)
- [x] 前端可见 24h 成功率环图 + Agent P95 趋势 (ai-health.tsx)
- [x] 超时审批自动 expired (expire_pending() + 5min aggregation_loop)

**U.10 推迟原因**：
- Playwright E2E 需要 OMLX:8000 / vLLM:8001 / API:8888 / Frontend:3000 全部在运行
- 当前 CLI 环境无这些外部服务
- 当前覆盖率 83.18%，≥ 80% gate 已通过，< 90% 留作 E2E 跑通后再守门
- 推荐操作：`make dev` 启动后跑 `npx playwright test`
