...
| # | 任务 | 文件 | 估时 |
|---|---|---|---:|
| T.1 | 精简 Prompt-H 到 80 行 | `app/agents/prompts/resume_parser.md`（新建）| 30 min |
| T.2 | 实现 `ResumeParsingAgent` 7-step 工作流 | `app/agents/resume_parser.py`（新建）| 3h |
| T.3 | RouterAgent 注册 `resume_parser` 意图 | `app/agents/router_agent.py` | 1h |
| T.4 | 迁移 `_BUILTIN_TOOLS` screening 部分到 `app/tools/screening.py` | `app/services/agent_service.py` | 1h |
| T.5 | 迁移 `_BUILTIN_TOOLS` interview 部分到 `app/tools/interview.py` | 同 | 1h |
| T.6 | `tests/test_tools/` 工具 handler 单测 | `apps/api/tests/test_tools/` | 2h |
| T.7 | `tests/test_resume_parser_agent.py` Agent 7-step 流程测 | 同 | 1h |
| T.8 | E2E：上传简历 → 解析 → 评估 跑通 |  | 1h |

**T 退出标准**：
- [ ] `POST /api/v1/router/classify` 文本包含"解析"→ 路由到 `resume_parser`
- [ ] `app/services/agent_service.py` 不再维护 `_BUILTIN_TOOLS`
- [ ] `app/tools/all_tools()` 返回全部工具定义
