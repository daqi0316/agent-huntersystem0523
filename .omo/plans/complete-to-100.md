# 整体收官计划 — AI Recruitment System 100%

> Goal: Push all dimensions to 100% — backend coverage 80%+, all frontend pages on real APIs, infrastructure production-ready, CI/CD pipeline validated.
...
**Files**: `tests/test_evaluations.py` (existing: test_dashboard_reports.py covers dashboard_reports, needs separate for evaluations)
...
- Mock `httpx.AsyncClient` via `unittest.mock.patch` or `resp` library
...
**File**: `tests/test_pipeline.py` or new `tests/test_gen_eval_loop.py`
...
**File**: `apps/api/app/llm/base.py` or new `apps/api/app/llm/retry.py`
...
Currently 3 files:
- `/Users/qixia/agent-huntersystem0523/.env.example` (root — 42 lines, has both API + web vars)
- `apps/api/.env.example` (48 lines, API-specific)
- `apps/web/.env.example` (7 lines, web-specific)

**Action**: Keep `apps/api/.env.example` as API source of truth; keep `apps/web/.env.example` as web source of truth; remove the redundant root `.env.example` (NEXT_PUBLIC_* vars already in web, DB/Redis/Qdrant already in api). Update root `README.md` quickstart to reference per-app files.
...
**Action**: Bump Dockerfiles to `python:3.14-slim-bookworm` (matches what CI/tests use)
...
**File**: Any other `utcnow()` occurrences
```bash
